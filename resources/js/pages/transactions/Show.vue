...
type TransactionShow = {
  ...
  from_account: MiniAccount | null;   // For asset/expense (source is Asset)
  from_ei_account: MiniAccount | null; // For income (source is E/I). If you prefer, just send one “source” object.

  to_account: MiniAccount | null;     // For asset/income (dest is Asset)
  to_ei_account: MiniAccount | null;  // For expense (dest is E/I)
...
const source = computed<MiniAccount | null>(() => {
  if (props.transaction.type === 'income') return props.transaction.from_ei_account;
  return props.transaction.from_account;
});
const destination = computed<MiniAccount | null>(() => {
  if (props.transaction.type === 'expense') return props.transaction.to_ei_account;
...
const sourceFeesTotal = computed(() =>
  (props.transaction.source_fees ?? []).reduce((sum, f) => sum + n(f.amount), 0),
);
const destFeesTotal = computed(() =>
  (props.transaction.dest_fees ?? []).reduce((sum, f) => sum + n(f.amount), 0),
);
    ...
    <div class="container mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
      ...
      <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div class="mt-2 flex flex-wrap items-center gap-2">
            ...
            <span class="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium dark:bg-gray-800" title="Created at">
      ...
      <div class="mb-6 grid gap-6 md:grid-cols-2">
        ...
        <section class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
          ...
          <dl class="grid grid-cols-3 gap-3 text-sm">
            ...
            <dd class="col-span-2">
              {{ source?.name ?? '—' }}
              <span v-if="sourceCurrency !== '—'" class="ml-2 rounded-md border border-gray-300 px-1 py-0.5 text-[10px]">{{ sourceCurrency }}</span>
            </dd>
            ...
            <dd class="col-span-2">
              <span class="font-mono">{{ transaction.send_actual_amount }}</span>
              <span v-if="sourceCurrency !== '—'">{{ ' ' + sourceCurrency }}</span>
            </dd>

            <dt class="text-gray-500 dark:text-gray-400">Send (Total)</dt>
            <dd class="col-span-2">
              <span class="font-mono">{{ transaction.send_total_amount }}</span>
              <span v-if="sourceCurrency !== '—'">{{ ' ' + sourceCurrency }}</span>
            </dd>
          </dl>
          ...
          <!-- Sender Fees -->
          <div class="mt-4">
            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Fees (Sender)</h3>
            <div v-if="(transaction.source_fees?.length ?? 0) === 0" class="text-xs text-gray-500">—</div>
            <ul v-else class="divide-y divide-gray-100 rounded-lg border border-gray-200 text-sm dark:divide-gray-800 dark:border-gray-800">
              <li
                v-for="(f, i) in transaction.source_fees"
                :key="`sf-${i}-${f.name}`"
        ...
        <section class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
          ...
          <dl class="grid grid-cols-3 gap-3 text-sm">
            ...
            <dd class="col-span-2">
              {{ destination?.name ?? '—' }}
              <span v-if="destCurrency !== '—'" class="ml-2 rounded-md border border-gray-300 px-1 py-0.5 text-[10px]">{{ destCurrency }}</span>
            </dd>
            ...
            <dd class="col-span-2">
              <span class="font-mono">{{ transaction.receive_actual_amount }}</span>
              <span v-if="destCurrency !== '—'">{{ ' ' + destCurrency }}</span>
            </dd>

            <dt class="text-gray-500 dark:text-gray-400">Receive (Total)</dt>
            <dd class="col-span-2">
              <span class="font-mono">{{ transaction.receive_total_amount }}</span>
              <span v-if="destCurrency !== '—'">{{ ' ' + destCurrency }}</span>
            </dd>
          </dl>
          ...
          <!-- Receiver Fees -->
          <div class="mt-4">
            <h3 class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Fees (Receiver)</h3>
            <div v-if="(transaction.dest_fees?.length ?? 0) === 0" class="text-xs text-gray-500">—</div>
            <ul v-else class="divide-y divide-gray-100 rounded-lg border border-gray-200 text-sm dark:divide-gray-800 dark:border-gray-800">
              <li
                v-for="(f, i) in transaction.dest_fees"
                :key="`df-${i}-${f.name}`"
                class="flex items-center justify-between px-3 py-2"
              >
                <span class="truncate">{{ f.name }}</span>
      ...
      <div class="grid gap-6 md:grid-cols-2">
        ...
        <section class="rounded-xl border border-gray-200 p-4 dark:border-gray-800">
          ...
          <div v-if="!props.transaction.attachments || props.transaction.attachments.length === 0" class="text-sm text-gray-500">
            —
          </div>
          <ul
            v-else
